...
def create_db():
    ...
    save_device_status(
        device_id="Device_001",        # 디바이스명
        power_status=1,                # 전원상태 (1: 켜짐)
        wear=1,                        # 착용상태 (1: 착용 중)
        active_time=datetime.now(),    # 동작시간 (현재 시간)
        last_time=datetime.now(),      # 마지막 동작시간 (현재 시간)
        emergency_icon=0               # 비상 상태 (0: 비상 아님)
    )
...
def update_device_status(device_id, power_status, emergency_icon, current_time):
    ...
    try:
        cursor.execute(
            "SELECT * FROM device_status WHERE 디바이스명 = ?", (device_id,))
